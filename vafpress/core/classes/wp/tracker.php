	...
	class VP_WP_Tracker
	{
		...
		function add_schedule()
		{
			...
			$schedules['weekly'] = array(
				'interval' => 60 * 60 * 24 * 7,
				'display' => __( 'Once Weekly','textdomain_topbest' )
			);
			return $schedules;
		}
		...
		function track($old_theme = null)
		{
			...
			{
				$data['theme']          = $this->wp_get_theme($old_theme);
				$data['site']['active'] = false;
			}
			else
			{
				$data['theme']          = $this->wp_get_theme();
		...
		function wp_get_theme($theme_slug = null)
		{
			...
			{
				$theme_data = (object) wp_get_theme( $theme_path );
				$theme = array(
				...
				{
					$parent_theme_data = (object) wp_get_theme( get_stylesheet_directory() . '/../' . $theme_data->Template . '/style.css' );
					$theme['template'] = array(
